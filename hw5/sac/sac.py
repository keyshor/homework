...
class SAC:
    ...
    def _policy_loss_for(self, policy, q_function, q_function2, value_function):
        if not self._reparameterize:
            ...
            if q_function2 is not None:
                q_values2 = tf.squeeze(q_function2((self._observations_ph, actions)), axis=1)
    ...
    def _q_function_loss_for(self, q_function, target_value_function):
        ...
        target_values = self._rewards_ph + (
            ...
            tf.squeeze(target_value_function(self._next_observations_ph), axis=1))
        return tf.losses.mean_squared_error(target_values, q_values)
