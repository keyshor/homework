...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        q_n = re_n + (1-terminal_n) * self.gamma * self.sess.run(self.critic_prediction,
                                                                 feed_dict={self.sy_ob_no: next_ob_no})
        adv_n = q_n - self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            target_n = re_n + (1-terminal_n) * self.gamma * self.sess.run(self.critic_prediction,
                                                                 feed_dict={self.sy_ob_no: next_ob_no})
            for _ in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={self.sy_target_n: target_n,
                                                                self.sy_ob_no: ob_no})
